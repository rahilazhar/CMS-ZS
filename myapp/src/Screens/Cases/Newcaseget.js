...
const Newcaseget = () => {
  ...
  const filteredItems = entriesAll.filter((entry) => {
  ...
  return (
    <>
      <main className="w-full text-center">
          ...
          <Box sx={style}>
            <div className=" flex flex-col gap-4">
              <Typography id="modal-modal-title">
                <span className=" font-bold">Nature of Suit</span> :{" "}
                {natureOfSuit}
              </Typography>
              <Typography id="modal-modal-title">
                <span className=" font-bold">Plaintiff's Background</span> :{" "}
                {plaintiffsBackground}
              </Typography>
              <Typography id="modal-modal-title">
                <span className=" font-bold">Plaintiff's Claim</span> :{" "}
                {plaintiffsClaim}
              </Typography>
              <Typography id="modal-modal-title">
                <span className=" font-bold">Defendant's Argument</span> :{" "}
                {defendantsArgument}
              </Typography>
              <Typography id="modal-modal-title">
              ...
              <Typography id="modal-modal-title">
                <span className=" font-bold">Plaintiff's Representation</span> :{" "}
                {plaintiffsRepresentation}
              </Typography>
              <Typography id="modal-modal-title">
                <span className=" font-bold">Defendant representative</span> :{" "}
                {defendantRepresentative}
              </Typography>
              <Typography id="modal-modal-title">
                <span className=" font-bold">Restraining Order</span> :{" "}
                {restrainingOrder}
              </Typography>
              ...
              <Typography id="modal-modal-title">
                <span className=" font-bold">
                  Additional Plaintiff Documents
                </span>
                :
                <ul className=" list-decimal ml-10">
              ...
              <Typography id="modal-modal-title">
                <span className=" font-bold">
                  Additional Defendant Documents
                </span>
                :
                <ul className=" list-decimal ml-10">
            ...
            <div className=" text-center font-bold text-xl underline mt-4">
              DATES
            </div>
          </Box>
        </Modal>
            ...
            <FormControl fullWidth>
              ...
              >
                ...
                {courtNames.map((courtName, index) => (
                  <MenuItem key={index} value={courtName}>
                    {courtName}
                  </MenuItem>
                ))}
              </Select>
        ...
        <div className=" bg-gray-200 shadow-xl p-4 font-semibold text-2xl">
          All Cases
        </div>
                ...
                return (
                  <>
                    ...
                    >
                      ...
                      <td className="px- py-4 whitespace-normal text-sm text-gray-500">
                        {entry.NatureofSuit ? entry.NatureofSuit : entry.nature}
                      </td>
                      <td className="px- py-4 whitespace-normal text-sm text-gray-500">
                        {entry.lastDateOfHearing
                          ? entry.lastDateOfHearing
                          : entry.prevhearing}
                      </td>
                      <td className="px- py-4 whitespace-normal text-sm text-gray-500">
                      ...
                      <td className="px-10 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <Link
                          to={`/viewdetails/${entry._id}/${entry.title}`}
                          className="text-gray-700 hover:text-indigo-900 text-xl"
                        >
                          <AiFillEye className="" />
                        </Link>
                      </td>
